...
wordpresscomModule.prototype.convertItem = function (item, type) {
    ...
    var firstImageUrl = 'ms-appx:/images/blank.png';
    var firstImageThumbUrl =  'ms-appx:/images/blank.png';;
    if (item.featured_image) {
        res.imgUrl = item.featured_image;
        res.imgThumbUrl = item.featured_image + "?h=220";  // TODO: resize based on CSS size?
    }
        ...
        for (var i in item.attachments) {
            if (item.attachments[i].URL) {
                if (index == 0) {
                    firstImageUrl = item.attachments[i].URL;
                    firstImageThumbUrl = item.attachments[i].URL + "?h=220";  // TODO: resize based on CSS size?
                }
                if (item.attachments[i].URL.indexOf("01-") > 0) {
                    firstImageUrl = item.attachments[i].URL;
                    firstImageThumbUrl = item.attachments[i].URL + "?h=220";  // TODO: resize based on CSS size?
                    break;
                }
    ...
    res.imgUrl = firstImageUrl;
    res.imgThumbUrl = firstImageThumbUrl;
